...
const CreateTask = () => {
  ...
  const taskStatus = searchParams.get("taskStatus") as
    | "pending"
    | "in-progress"
    | "done";
  const { setTasks } = useTaskStore((state: any) => state);
  ...
  const { isLoading, setIsLoading, setToastData } = useTaskStore(
    ...
    <div className="flex-col w-full min-h-dvh bg-[#F7F9F2] text-[#10439F] gap-4">
      <div>
        <div className="ml-20 pt-16">
          <button
            type="button"
            className="text-7xl w-8 h-8 hover:cursor-pointer"
            onClick={() => router.back()}
          >
            <ArrowCircleLeftIcon style={{ transform: "scale(2)" }} />
          </button>
        </div>
        <h2 className="mb-4 text-[#10439F]">Create New Task</h2>
      </div>
      <div className="w-full min-h-full justify-center items-center">
        <form
          className="border rounded-md bg-gray-200 flex-col gap-5 p-2 my-0 mx-auto min-h-96 w-1/2 text-center font-bold"
        ...
        >
          <div className="mb-36">
            ...
            <div className="gap-7 mb-5 w-full">
              ...
              <textarea
                ...
                onChange={handleInput}
                required
                className="font-normal outline-0 block p-2.5 w-full text-sm bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div className="flex gap-5 mb-0 mt-5 w-full justify-center items-center">
              ...
              {isLoading ? (
                <div>
                  <svg
                    aria-hidden="true"
                    role="status"
                    className="inline w-7 h-7 me-3 text-gray-200 animate-spin dark:text-white-600"
                    viewBox="0 0 100 101"
                    fill="none"
            ...
            <button className="outline p-3 rounded w-1/3" type="reset">
              Reset
            </button>
          </div>
        </form>
