...
const EditTask = () => {
  const initTaskValues = INITIAL_TASK_VALUE;
  const router = useRouter();
  const params = useParams();
  const { taskId } = params;

  const { isLoading, setIsLoading, setToastData, setTasks } = useTaskStore(
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    router.push("/");
    if (data) {
      const toastData = SUCCESS_UPDATE_TOAST_DATA;
